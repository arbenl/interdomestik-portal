...
describe('HTTP functions', () => {
  ...
  it('stripeWebhook writes a paid invoice', async () => {
    ...
    await admin
      .auth()
      .createUser({ uid, email: `u_${Date.now()}@example.com` })
      .catch(() => {});
    ...
    const req: any = {
      method: 'POST',
      body: { uid, invoiceId, amount: 2500, currency: 'EUR' },
    };
    const { res, get } = makeRes();
