...
import { getPortalDashboard, getPortalLayout, upsertPortalLayout } from '../src/index';

const testEnv = functionsTest({ projectId: 'interdomestik-dev' });

describe('portal dashboard callables', () => {
  const adminCtx = { auth: { uid: 'admin-dashboard', token: { role: 'admin', allowedRegions: ['PRISHTINA', 'PEJA'] } } } as any;
  const memberCtx = { auth: { uid: 'member-dashboard', token: { role: 'member' } } } as any;

  before(async () => {
    const now = Date.now();
    const expiresSoon = admin.firestore.Timestamp.fromDate(new Date(now + 15 * 24 * 60 * 60 * 1000));
    ...
    await db.collection('audit_logs').add({
      ...
      currency: 'EUR',
      region: 'PRISHTINA',
      ts: admin.firestore.Timestamp.fromDate(new Date(now - 3 * 24 * 60 * 60 * 1000)),
    });
    await db.collection('audit_logs').add({
      action: 'startMembership',
      amount: 100,
      currency: 'EUR',
      region: 'PRISHTINA',
      ts: admin.firestore.Timestamp.fromDate(new Date(now - 10 * 24 * 60 * 60 * 1000)),
    });

    await db.collection('events').add({
      title: 'Compliance Summit',
      startAt: admin.firestore.Timestamp.fromDate(new Date(now + 5 * 24 * 60 * 60 * 1000)),
  ...
  it('returns aggregated widget data for admins', async () => {
    ...
    const payments = result.widgets.find((w: any) => w.id === 'paymentsCaptured');
  ...
  it('persists a sanitized layout', async () => {
    ...
    const saved = await wrapUpsert({ widgets: [{ id: 'churnRisk' }, { id: 'renewalsDue' }, { id: 'churnRisk' }] }, adminCtx) as any;
    expect(saved.widgets[0].id).to.equal('churnRisk');
    expect(saved.widgets.map((w: any) => w.id)).to.include.members(['renewalsDue', 'eventRegistrations', 'paymentsCaptured']);

    const doc = await db.collection('portalLayouts').doc('admin-dashboard').get();
