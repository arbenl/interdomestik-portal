...
describe('exportsV2 helpers', () => {
  ...
  before(async () => {
    await admin
      .auth()
      .createUser({ uid, email: `exp_${Date.now()}@example.com` })
      .catch(() => {});
    ...
    await db.collection('members').doc('m1').set({
      memberNo: 'INT-2025-000010',
      name: 'A',
      email: 'a@ex.com',
      region: 'PRISHTINA',
      status: 'active',
    ...
    await db.collection('members').doc('m3').set({
      memberNo: 'INT-2025-000012',
      name: 'C',
      email: 'c@ex.com',
      region: 'GJAKOVA',
      status: 'active',
  ...
  it('buildMembersQuery intersects admin allowedRegions with filter', async () => {
    const q = buildMembersQuery({ regions: ['PRISHTINA', 'GJAKOVA'] } as any, [
      'PRISHTINA',
      'PEJA',
    ]);
  ...
  it('streamMembersCsv writes CSV to storage and reports rows/size', async () => {
    ...
    try {
      const res = await streamMembersCsv({
        exportId: 't1',
        filters: { regions: ['PRISHTINA', 'PEJA'] } as any,
        columns: normalizeColumns(['memberNo', 'name', 'region'], 'BASIC'),
        actorUid: uid,
