...
describe('payments: createPaymentIntentLogic (emulator)', () => {
  ...
  before(async () => {
    ...
    await db
      .collection('coupons')
      .doc('save10')
      .set({ percentOff: 10, active: true });
    await db
      .collection('coupons')
      .doc('off500')
      .set({ amountOff: 500, active: true });
  });

  it('applies donation and percent coupon', async () => {
    const r: any = await createPI(
      { amount: 2500, currency: 'EUR', donateCents: 200, couponCode: 'save10' },
      ctx
    );
  ...
  it('applies amountOff coupon if larger', async () => {
    const r: any = await createPI(
      { amount: 2500, currency: 'EUR', donateCents: 0, couponCode: 'off500' },
      ctx
    );
    expect(r.ok).to.equal(true);
    // 2500 - 500 = 2000
