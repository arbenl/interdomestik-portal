...
export async function startMembershipLogic(data: any, context: functions.https.CallableContext) {
  ...
  try {
    ...
    if (exists.exists) {
      ...
      return { message: "Already processed", refPath: prev.refPath, idempotent: true };
    }
  } catch (e) {
  ...
  try {
    await idemRef.create({ uid, year, source, refPath, ts: FieldValue.serverTimestamp(), externalRef: externalRef ?? null });
    createdIdem = true;
  } catch (e: any) {
  ...
  const html = membershipCardHtml({ memberNo, name, region, validity: `${year}`, verifyUrl });
  ...
  if (createdIdem && !alreadyActive) {
    ...
    } catch (e) { log('start_membership_email_error', { uid, year, error: String(e) }); }
  }
  ...
  if (createdIdem && !alreadyActive && (price ?? 0) > 0) {
    ...
    } catch (e) { log('start_membership_receipt_error', { uid, year, error: String(e) }); }
  }
  ...
  if (createdIdem) {
    try {
      ...
      log('audit_write_failed', { action: 'startMembership', uid, error: String(e) });
    }
  }
  ...
  if (createdIdem) {
    try {
      ...
      log('metrics_write_failed', { metric: 'daily_activations', uid, region: String(region || 'UNKNOWN'), error: String(e) });
    }
  }
