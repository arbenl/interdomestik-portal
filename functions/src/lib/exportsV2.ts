...
export type AllowedColumn =
  | (typeof BASIC_COLUMNS)[number]
  | (typeof FULL_ADMIN_COLUMNS)[number];
...
export function buildMembersQuery(
  ...
  const filterRegions = Array.isArray(filters.regions)
    ? filters.regions.filter(Boolean)
    : [];
  const adminRegions = Array.isArray(allowedRegions)
    ? allowedRegions.filter(Boolean)
    : [];
  ...
  if (regionsToUse.length > 0)
    q = q.where('region', 'in', regionsToUse.slice(0, 10));
  if (filters.status) q = q.where('status', '==', filters.status);
  if (filters.orgId) q = q.where('orgId', '==', filters.orgId);
  if (filters.expiringAfter)
    q = q.where('expiresAt', '>=', filters.expiringAfter);
  if (filters.expiringBefore)
    q = q.where('expiresAt', '<=', filters.expiringBefore);
...
export async function streamMembersCsv({
  ...
  const allowedRegions =
    (user?.customClaims?.allowedRegions as string[] | undefined) || undefined;
      ...
      for (const col of columns) {
        ...
        switch (col) {
          ...
          case 'year':
            ...
            break;
          case 'expiresAt':
            v = m.expiresAt?.toDate
              ? m.expiresAt.toDate().toISOString().slice(0, 10)
              : '';
            break;
          case 'agentId':
            v = m.agentId;
            break;
          case 'createdAt':
            v = m.createdAt?.toDate ? m.createdAt.toDate().toISOString() : '';
            break;
          case 'updatedAt':
            v = m.updatedAt?.toDate ? m.updatedAt.toDate().toISOString() : '';
            break;
          case 'active':
            v =
              m.status === 'active' ||
  ...
  try {
    const [signed] = await file.getSignedUrl({
      action: 'read',
      expires: Date.now() + 3 * 24 * 3600 * 1000,
    });
    url = signed;
  } catch {}
