...
export async function shareDocumentLogic(
  rawData: unknown,
  ...
  if (!isAdmin && !isAgent) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only admins or agents may share documents.'
    );
  }

  const parsed = shareDocumentSchema.parse(rawData);
  const recipientUids = Array.from(
    new Set(parsed.recipients.map((r) => r.uid))
  );
  ...
  if (isAgent) {
    const agentRegions = Array.isArray(actorClaims.allowedRegions)
      ? actorClaims.allowedRegions
      : [];
    if (agentRegions.length === 0) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Agent has no allowed regions configured.'
      );
    }
    for (const { data } of memberDocs) {
      ...
      if (!memberRegion || !agentRegions.includes(memberRegion)) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Agent cannot share documents outside their allowed regions.'
        );
      }
    }
  }
  ...
  const recipientsDetailed: RecipientRecord[] = memberDocs.map(
    ({ uid, data }) => ({
      uid,
  ...
  await db.runTransaction(async (txn) => {
    ...
    const ownerUid = existing.exists
      ? (existing.get('ownerUid') as string | undefined)
      : undefined;
    if (existing.exists && ownerUid && ownerUid !== auth.uid && !isAdmin) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only the owner or an admin can update this document share.'
      );
    }

    const baseData: Record<string, unknown> = {
      ownerUid: ownerUid ?? auth.uid,
      ownerRole: existing.exists
        ? (existing.get('ownerRole') ?? actorRole)
        : actorRole,
