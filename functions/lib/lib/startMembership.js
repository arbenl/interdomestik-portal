...
async function activateMembership(
  ...
  const expiresAt = firestore_1.Timestamp.fromDate(
...
async function startMembershipLogic(data, context) {
  ...
  const { uid, year, price, currency, paymentMethod, externalRef } =
    validators_1.startMembershipSchema.parse(data);
  ...
  try {
    ...
    if (exists.exists) {
      ...
      (0, logger_1.log)('start_membership_idempotent_hit', {
        uid,
        year,
        source,
      });
  ...
  if (createdIdem && !alreadyActive) {
    try {
      ...
      (0, logger_1.log)('start_membership_email_error', {
        uid,
        year,
        error: String(e),
      });
  ...
  if (createdIdem && !alreadyActive && (price ?? 0) > 0) {
    try {
      ...
      (0, logger_1.log)('start_membership_receipt_error', {
        uid,
        year,
        error: String(e),
      });
  ...
  if (createdIdem) {
    try {
      await firebaseAdmin_1.db.collection('audit_logs').add({
        ...
        method: paymentMethod,
        ts: firestore_1.FieldValue.serverTimestamp(),
        ttlAt: firestore_1.Timestamp.fromDate(
  ...
  if (createdIdem) {
    try {
      ...
      const ref = firebaseAdmin_1.db
        ...
        .doc(`daily-${dateKey}`);
      const inc =
        firebaseAdmin_1.admin.firestore.FieldValue.increment?.(1) ||
      ...
      const ttlAt = firestore_1.Timestamp.fromDate(
