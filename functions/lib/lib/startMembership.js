...
async function startMembershipLogic(data, context) {
    ...
    try {
        await idemRef.create({ uid, year, source, refPath, ts: firestore_1.FieldValue.serverTimestamp(), externalRef: externalRef ?? null });
        createdIdem = true;
    }
    catch (e) {
    ...
    const html = (0, membership_1.membershipCardHtml)({ memberNo, name, region, validity: `${year}`, verifyUrl });
    ...
    if (createdIdem) {
        try {
            ...
            (0, logger_1.log)('audit_write_failed', { action: 'startMembership', uid, error: String(e) });
        }
    }
    ...
    if (createdIdem) {
        try {
            ...
            (0, logger_1.log)('metrics_write_failed', { metric: 'daily_activations', uid, region: String(region || 'UNKNOWN'), error: String(e) });
        }
    }
