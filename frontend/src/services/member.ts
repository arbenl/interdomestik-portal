...
export const getMembershipHistory = async (uid: string): Promise<Membership[]> => {
  const historyCollection = collection(firestore, 'members', uid, 'memberships');
...
export const getCardToken = async (uid: string): Promise<string> => {
  const getCardToken = httpsCallable<{ uid: string }, { token: string }>(functions, 'getCardToken');
