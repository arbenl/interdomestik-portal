...
export const PORTAL_WIDGET_METADATA: Record<PortalWidgetId, { title: string; description: string }> = {
  renewalsDue: {
...
const defaultLayout: PortalLayoutItem[] = Object.keys(PORTAL_WIDGET_METADATA).map(id => ({ id: id as PortalWidgetId }));

export const SUPPORTED_WIDGET_IDS: PortalWidgetId[] = defaultLayout.map(item => item.id);
...
function normalizeDashboardResponse(response: PortalDashboardResponse): PortalDashboardResponse {
