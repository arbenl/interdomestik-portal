...
export default function PortalHero({
  ...
  const chip = (() => {
    ...
    if (s === 'expired')
      return { label: 'EXPIRED', cls: 'bg-yellow-100 text-yellow-800' };
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 text-sm text-gray-600">
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-semibold ${chip.cls}`}
            >
              {chip.label}
            </span>
            {memberNo && (
              <span className="font-mono">Member No: {memberNo}</span>
            )}
        ...
        {verifyUrl && (
          <div className="hidden sm:flex items-center gap-3">
            ...
            <div className="text-xs text-gray-500 max-w-[220px]">
              ...
              <a className="underline break-all" href={verifyUrl}>
                {verifyUrl}
              </a>
            </div>
          </div>
