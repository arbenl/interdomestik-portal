...
describe('Billing page', () => {
  ...
  it('simulates a payment successfully', async () => {
    ...
    renderWithProviders(<Billing />);
    const button = screen.getByRole('button', { name: /Add test paid invoice/i });
    await act(async () => {
      fireEvent.click(button);
      await vi.runAllTimersAsync();
    });
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:5001/interdomestik-dev/europe-west1/stripeWebhook', expect.any(Object));
    expect(refetchMock).toHaveBeenCalled();
    expect(pushMock).toHaveBeenCalledWith({ type: 'success', message: 'Payment recorded. Membership activated.' });
  ...
  it('surfaces errors when the payment simulation fails', async () => {
    ...
    await waitFor(() => {
      expect(pushMock).toHaveBeenCalledWith({ type: 'error', message: 'Failed to simulate payment' });
    });
  ...
  it('renders invoice rows with formatted amounts', async () => {
    (useInvoices as Mock).mockReturnValue({
      data: [{ id: 'inv1', amount: 1234, currency: 'XYZ', status: 'paid', created: { seconds: 1700000000 } }],
      isLoading: false,
      error: null,
      refetch: refetchMock,
    });
