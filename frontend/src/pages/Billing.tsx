...
import {
  functions,
  projectId as firebaseProjectId,
  emulatorProjectId,
} from '@/lib/firebase';
...
function getStripeWebhookUrl(projectId: string, emulatorProjectId: string) {
  ...
  if (isLocal && location.port === '5000')
    return `${location.origin}/stripeWebhook`;
  // Otherwise, call Functions emulator directly
  if (isLocal)
    return `http://localhost:5001/${emulatorProjectId}/europe-west1/stripeWebhook`;
...
export default function Billing() {
  ...
  const ENABLE_PAYMENTS_UI =
    String(import.meta.env.VITE_ENABLE_PAYMENTS_UI ?? 'true') === 'true';
