...
export default function PortalDocuments() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (!fileName.trim() || !storagePath.trim() || recipientList.length === 0) {
      push({ type: 'error', message: 'Provide file name, storage path, and at least one recipient UID.' });
      return;
    }
    setSubmitting(true);
    try {
      ...
      push({ type: 'error', message: 'Failed to share document. Please try again.' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <PortalShell
      header={
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-500">Securely distribute policy updates, invoices, or identity files to individual members.</p>
        </div>
      }
    >
      {canShare ? (
        <section aria-labelledby="share-form" className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 id="share-form" className="text-lg font-semibold text-gray-900">Share a document</h2>
            {isFetching ? <span className="text-xs text-gray-400">Refreshing…</span> : null}
          </div>
      ...
      <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm" aria-labelledby="documents-shared">
        <div className="mb-4 flex items-center justify-between">
          <h2 id="documents-shared" className="text-lg font-semibold text-gray-900">Shared documents</h2>
          {shares.length ? <span className="text-xs text-gray-500">{shares.length} item{shares.length === 1 ? '' : 's'}</span> : null}
        </div>
        {error ? <p className="mb-3 text-sm text-red-600">Unable to load shared documents. Try refreshing.</p> : null}
        {shares.length === 0 ? (
                    ...
                    <Fragment key={share.id}>
                      <tr className="border-b last:border-none">
                        <td className="px-3 py-2">
                          <div className="font-medium text-gray-900">{share.fileName}</div>
                          <div className="text-xs text-gray-500">{share.storagePath}</div>
                        </td>
                        <td className="px-3 py-2 text-gray-700">{share.ownerUid}</td>
                        <td className="px-3 py-2">
                          <ul className="space-y-1 text-gray-700">
                            {share.recipients.map(recipient => (
                              <li key={`${share.id}-${recipient.uid}`} className="text-xs">
                                <span className="font-medium">{recipient.uid}</span>
                        ...
                        <td className="px-3 py-2 text-gray-700">{formatDate(share.updatedAt)}</td>
                        <td className="px-3 py-2">
                          <Button
                            variant="ghost"
...
function DocumentActivityList({ shareId }: { shareId: string }) {
  ...
  return (
    <ul className="space-y-2 text-xs text-gray-700">
      {items.map(item => (
        <li key={item.id} className="rounded-md border border-gray-200 bg-white p-2 shadow-sm">
          ...
          {item.recipients.length > 0 ? (
            <div className="text-gray-600">Recipients: {item.recipients.join(', ')}</div>
          ) : null}
          {item.note ? <div className="text-gray-600">Note: {item.note}</div> : null}
        </li>
      ))}
    </ul>
  );
