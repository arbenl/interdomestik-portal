...
export function useOfflineCard(liveToken: string | null) {
  ...
  const [cachedExp, setCachedExp] = useState<number | null>(() => {
    try {
      const v = localStorage.getItem(LS_EXP);
  ...
  useEffect(() => {
    ...
    setCachedExp(exp);
    try {
      localStorage.setItem(LS_TOKEN, liveToken);
    } catch {
      /* ignore */
    }
    try {
  ...
  const effectiveToken = useMemo(
    () => liveToken || cachedToken || null,
    [liveToken, cachedToken]
  );
  const expSec = useMemo(
    () => parseJwtExp(effectiveToken) || cachedExp || null,
    [effectiveToken, cachedExp]
  );
  const nowSec = Math.floor(Date.now() / 1000);
  const secondsToExpiry = typeof expSec === 'number' ? expSec - nowSec : null;
  const nearExpiry =
    typeof secondsToExpiry === 'number'
      ? secondsToExpiry < 7 * 24 * 3600
      : false; // 7 days
