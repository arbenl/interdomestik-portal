...
const searchUserByEmail = httpsCallable<{ email: string }, { uid: string }>(
  functions,
  'searchUserByEmail'
);
const setUserRole = httpsCallable<
  { uid: string; role: Role; allowedRegions: string[] },
  { message: string }
...
export function useRoleManager() {
  ...
  const {
    mutate: applyRole,
    isPending: isApplyingRole,
    error: applyRoleError,
  } = useMutation({
    mutationFn: async ({
      uid,
      role,
  ...
  const {
    mutate: viewClaims,
    isPending: isViewingClaims,
    error: viewClaimsError,
  } = useMutation({
  ...
  return {
    busy:
      isFindingUser || isApplyingRole || isViewingClaims || isRefreshingToken,
    error:
      findUserError?.message ||
      applyRoleError?.message ||
      viewClaimsError?.message ||
