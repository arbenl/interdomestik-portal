...
export function RoleManagerPanel() {
  const { busy, error, success, claims, targetUid, findUser, applyRole, viewClaims, setTargetUid, refreshToken } = useRoleManager();
  ...
  return (
    <div className="mb-6 p-4 border rounded bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        ...
        <Button onClick={() => { void findUser(lookupEmail); }} disabled={busy}>Find UID</Button>
        <div>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <select className="mt-1 w-full border rounded px-3 py-2" value={role} onChange={e => setRole(e.target.value as 'member' | 'agent' | 'admin')}>
          ...
          <div className="mt-1 grid grid-cols-2 md:grid-cols-4 gap-2">
            {REGIONS.map(r => (
              <label key={r} className="inline-flex items-center gap-2 text-sm">
                <input type="checkbox" checked={regions.includes(r)} onChange={(e) => {
                  setRegions(prev => e.target.checked ? Array.from(new Set([...prev, r])) : prev.filter(x => x !== r));
                }} />
                <span>{r}</span>
              </label>
            ))}
      ...
      <div className="mt-4">
        <Button onClick={() => { void applyRole({ uid: targetUid, role, allowedRegions: regions }); }} disabled={!targetUid || busy}>Apply Role</Button>
        <Button onClick={() => { void viewClaims(targetUid); }} disabled={!targetUid || busy} variant="ghost">View Claims</Button>
        <Button onClick={() => { void refreshToken(); }} variant="ghost">Refresh my token</Button>
        {claims && (
