...
export function MetricsPanel() {
  ...
  const assistantStats = useMemo(() => {
    ...
    for (const session of sessions) {
      ...
      if (
        session.updatedAt &&
        now - session.updatedAt.getTime() <= 24 * 60 * 60 * 1000
      ) {
        sessions24h += 1;
      }
    }
  ...
  const latencyStats = useMemo(() => {
    ...
    if (sampleSize === 0) {
      ...
      const fallbackRate =
        entries.length === 0 ? null : (fallbackCount / entries.length) * 100;
      return {
        averageLatencyMs: null,
        p95LatencyMs: null,
        fallbackRatePercent:
          fallbackRate === null ? null : Math.round(fallbackRate * 10) / 10,
    ...
    const p95Index = Math.min(
      sampleSize - 1,
      Math.max(0, Math.ceil(0.95 * sampleSize) - 1)
    );
    const p95LatencyMs = Math.round(latencies[p95Index]);
    const fallbackCount = entries.filter((entry) => entry.fallback).length;
    const fallbackRatePercent =
      Math.round((fallbackCount / entries.length) * 1000) / 10;
  ...
  return (
    <div className="space-y-6">
      <section
        aria-labelledby="automation-dashboard"
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Latest renewal webhook dispatches. Monitor status codes and
              volume.
            </p>
          </div>
                ...
                {automationQuery.data?.logs.map((log) => (
                  <tr key={log.id} className="border-b last:border-none">
                    <td className="px-3 py-2 text-gray-700">
                      <div className="font-medium break-all text-indigo-700">
                        {log.url}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusVariant(log.status)}`}
                      >
                        {log.status}
                      </span>
                    </td>
        ...
        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between">
            ...
            {automationAlertsQuery.isLoading ? (
              <span className="text-xs text-gray-400">Loading…</span>
            ) : null}
          </div>
          {automationAlertsQuery.isError ? (
            <p className="text-sm text-red-600">
              Failed to load automation alerts.
            </p>
          ) : null}
      ...
      <section
        aria-labelledby="assistant-telemetry"
        className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm"
      >
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Session volume and recent prompts help you understand assistant
              engagement.
            </p>
          </div>
        ...
        {assistantQuery.isError ? (
          <p className="text-sm text-red-600">
            Failed to load assistant telemetry.
          </p>
        ) : null}
        {(assistantQuery.data?.sessions?.length ?? 0) === 0 ? (
          <p className="text-sm text-gray-500">
            No assistant sessions recorded yet.
          </p>
        ) : (
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {latencyStats.averageLatencyMs != null
                      ? `${latencyStats.averageLatencyMs} ms`
                      : '—'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>P95</span>
                  <span>
                    {latencyStats.p95LatencyMs != null
                      ? `${latencyStats.p95LatencyMs} ms`
                      : '—'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Fallback</span>
                  <span>
                    {latencyStats.fallbackRatePercent != null
                      ? `${latencyStats.fallbackRatePercent.toFixed(1)}%`
                      : '—'}
                  </span>
                </div>
              </dl>
              <p className="mt-2 text-xs text-gray-500">
                Sample size: {latencyStats.sampleSize}
              </p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
              <p className="text-xs uppercase text-gray-500">Role breakdown</p>
              <ul className="mt-1 space-y-1 text-sm text-gray-700">
                {Array.from(assistantStats.roles.entries()).map(
                  ([role, count]) => (
