...
let testEnv: Awaited<ReturnType<typeof initializeTestEnvironment>> | null =
  null;
const FIRESTORE_HOST = process.env.FIRESTORE_EMULATOR_HOST ?? '';

if (!FIRESTORE_HOST) {
  console.warn(
    '[rules-test] FIRESTORE_EMULATOR_HOST not set; skipping Firestore rules tests.'
  );
}
...
beforeAll(async () => {
  ...
  await testEnv.withSecurityRulesDisabled(async (context) => {
    ...
    await setDoc(doc(db, 'members', 'member-uid'), {
      name: 'Member',
      region: 'EU',
    });
    await setDoc(doc(db, 'members', 'other-uid'), {
      name: 'Other',
      region: 'EU',
    });
    await setDoc(doc(db, 'members', 'member-uid', 'memberships', '2025'), {
      status: 'active',
      year: 2025,
    });
    await setDoc(doc(db, 'billing', 'member-uid', 'invoices', 'inv1'), {
      amount: 2500,
      currency: 'EUR',
      status: 'paid',
    });
    await setDoc(doc(db, 'metrics', 'daily-2025-09-22'), {
      activations_total: 1,
    });
    ...
    await setDoc(doc(db, 'exports', 'job-1'), {
      status: 'running',
      type: 'members',
      startedAt: { seconds: 1 },
    });
    await setDoc(doc(db, 'assistantSessions', 'member-uid'), {
      uid: 'member-uid',
      updatedAt: { seconds: 1 },
    });
    await setDoc(doc(db, 'assistantSessions', 'member-uid', 'messages', 'm1'), {
      role: 'assistant',
      content: 'Hello',
      createdAt: { seconds: 1 },
    });
    await setDoc(doc(db, 'portalLayouts', 'member-uid'), {
      widgets: [{ id: 'renewalsDue' }],
    });
...
describeRules('Firestore rules (aligned to app data model)', () => {
  ...
  it('owner can read memberships, cannot write; admin writes blocked client-side', async () => {
    ...
    await assertSucceeds(
      getDoc(doc(db, 'members', 'member-uid', 'memberships', '2025'))
    );
    await assertFails(
      setDoc(doc(db, 'members', 'member-uid', 'memberships', '2025'), {
        status: 'expired',
      })
    );
    ...
    await assertFails(
      setDoc(doc(dbAdmin, 'members', 'member-uid', 'memberships', '2026'), {
        status: 'active',
      })
    );
  });
  ...
  it('billing invoices: owner and admin can read; no client writes', async () => {
    ...
    const db = memberCtx.firestore();
    await assertSucceeds(
      getDoc(doc(db, 'billing', 'member-uid', 'invoices', 'inv1'))
    );
    await assertFails(
      setDoc(doc(db, 'billing', 'member-uid', 'invoices', 'inv2'), {
        amount: 100,
      })
    );
    ...
    const dbAdmin = adminCtx.firestore();
    await assertSucceeds(
      getDoc(doc(dbAdmin, 'billing', 'member-uid', 'invoices', 'inv1'))
    );
  });
  ...
  it('exports: only admins can list or create', async () => {
    ...
    await assertFails(
      addDoc(collection(dbMember, 'exports'), { status: 'running' })
    );
    ...
    await assertSucceeds(
      addDoc(collection(dbAdmin, 'exports'), {
        status: 'running',
        type: 'members',
      })
    );
  ...
  it('portal layouts: owners and admins can read/write, others blocked', async () => {
    ...
    await assertSucceeds(
      setDoc(doc(dbMember, 'portalLayouts', 'member-uid'), {
        widgets: [{ id: 'renewalsDue' }, { id: 'churnRisk', hidden: true }],
      })
    );
    await assertFails(
      setDoc(doc(dbMember, 'portalLayouts', 'other-uid'), { widgets: [] })
    );
    ...
    await assertSucceeds(
      setDoc(doc(dbAdmin, 'portalLayouts', 'member-uid'), {
        widgets: [{ id: 'eventRegistrations' }],
      })
    );
  ...
  it('assistant sessions: owners and admins can read, no client writes', async () => {
    ...
    const dbMember = memberCtx.firestore();
    await assertSucceeds(
      getDoc(doc(dbMember, 'assistantSessions', 'member-uid'))
    );
    await assertSucceeds(
      getDoc(doc(dbMember, 'assistantSessions', 'member-uid', 'messages', 'm1'))
    );
    await assertFails(
      setDoc(doc(dbMember, 'assistantSessions', 'member-uid'), {
        role: 'member',
      })
    );
    await assertFails(
      setDoc(
        doc(dbMember, 'assistantSessions', 'member-uid', 'messages', 'm2'),
        { role: 'user' }
      )
    );
    ...
    const dbAdmin = adminCtx.firestore();
    await assertSucceeds(
      getDoc(doc(dbAdmin, 'assistantSessions', 'member-uid'))
    );
  ...
  it('document shares: owner, recipients, and admins can read metadata and activity', async () => {
    ...
    await assertSucceeds(
      getDoc(doc(ownerDb, 'documentShares', 'share-1', 'activity', 'event-1'))
    );
    ...
    await assertSucceeds(
      getDoc(
        doc(recipientDb, 'documentShares', 'share-1', 'activity', 'event-1')
      )
    );
